...
class roborock_mqtt_connector {
	...
	async initMQTT_Subscribe(rriot) {
		...
		await client.on("error", (result) => {
			this.adapter.catchError(`MQTT connection error: ${result}. rriot.r.m: ${rriot.r.m} rriot.u: ${rriot.u}, mqttUser: ${mqttUser}, mqttPassword: ${mqttPassword}`, `client.on("error")`);
			...
			try {
				...
				if (data.protocol == 102) {
					...
					if (typeof JSON.parse(data.payload).dps["102"] != "undefined") {
						...
						// emit this unknown commands, useful for customizing
						this.adapter.emit("unknownMessage102", duid, dps);
					}
	...
	sendMessage(duid, roborockMessage) {
		client.publish(`rr/m/i/${this.adapter.userdata.rriot.u}/${mqttUser}/${duid}`, roborockMessage, { qos: 1 });
	}
